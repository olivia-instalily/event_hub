...
// supabase/functions/create-luma/index.ts
...
// CDN (^https://images.lumacdn.com/...), so an external URL (our GCS cover) must be uploaded first:
// ask Luma for an upload URL, PUT the bytes, then use the returned file_url. Best-effort — returns
// null on any failure so the event is still created (just without the cover).
