...
import { fundingFor, leadTimeCheck, type ScopingForm as ScopingData } from "../lib/scoping";
...
export function composeScoping(base: ScopingData, plan: EventPlanning): ScopingData {
  ...
  return {
    ...
    components: base.components.length ? base.components : plan.engagements.map((e) => e.category ?? "").filter(Boolean),
    strategicJustification: base.strategicJustification || draftJustification(plan, funding, 0),
    generated: true,
  };
}

const Auto = () => <span className="ml-1.5 align-middle text-[10px] uppercase tracking-wide text-gray-400 bg-gray-100 rounded px-1 py-0.5">auto</span>;
...
export function ScopingForm({ plan, scoping, roughTotal, onChange, onClose }: {
  ...
  const buildSummary = (): string => [
    `Scoping — ${plan.title}`,
    `Date: ${plan.date ?? "TBD"}${lead.days != null ? ` (${lead.days}d lead${lead.ok ? "" : " — under 30d ⚠"})` : ""}`,
    ...
    `Rough cost: ${money(roughTotal)}${perPerson != null ? ` (${money(perPerson)}/person)` : ""}`,
    scoping.execSponsor ? `Exec sponsor: ${scoping.execSponsor}` : "",
    "",
    `Justification: ${scoping.strategicJustification}`,
  ].filter(Boolean).join("\n");

  const submit = () => set({ status: "submitted", submittedSummary: buildSummary() });
  ...
  return createPortal(
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 p-4" onClick={onClose}>
      <div className="bg-white rounded-2xl border border-black max-w-2xl w-full max-h-[88vh] overflow-y-auto p-6" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <label className="block">
            <span className="text-sm text-gray-600 mb-1 block">Audience<Auto /></span>
          ...
          <label className="block">
            <span className="text-sm text-gray-600 mb-1 block">Headcount<Auto /></span>
          ...
          <label className="block">
            <span className="text-sm text-gray-600 mb-1 block">Venue<Auto /></span>
          ...
          <label className="block sm:col-span-2">
            <span className="text-sm text-gray-600 mb-1 block">Components<Auto /></span>
        ...
        <div className="mt-5 pt-4 border-t border-gray-100 flex items-center justify-between gap-3">
          {scoping.status === "draft" ? (
            <>
              <span className="text-xs text-gray-400">{required ? "Ready to submit." : "Fill type, audience, headcount & justification to submit."}</span>
              <button onClick={submit} disabled={!required} className="inline-flex items-center gap-1.5 px-4 py-2 bg-gray-900 text-white rounded-lg text-sm hover:bg-gray-800 disabled:opacity-40">Submit for approval <Send className="w-4 h-4" /></button>
            </>
          ) : scoping.status === "submitted" ? (
            <>
              <span className="text-sm text-amber-700 inline-flex items-center gap-1.5"><Check className="w-4 h-4" /> Submitted — awaiting budget assignment.</span>
              <button onClick={reopen} className="text-sm text-gray-600 hover:text-gray-900">Reopen draft</button>
            </>
          ) : (
            <span className="text-sm text-green-700 inline-flex items-center gap-1.5"><Lock className="w-4 h-4" /> Budget assigned & locked.</span>
          )}
        </div>
      </div>
    </div>,
