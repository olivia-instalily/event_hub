...
export function LabelPicker({
  ...
  return (
    <div className="flex flex-wrap items-center gap-2">
      {assigned.map((l) => (
        <span key={l.id} className="inline-flex items-center gap-1 px-2 py-1 rounded-full text-xs bg-gray-100 text-gray-700">
          {l.name}
      ...
      <div className="relative">
        ...
        {open && (
          <div className="absolute z-20 mt-1 w-52 bg-white border border-black rounded-lg shadow-lg p-2">
            {available.map((l) => (
              <button key={l.id} onClick={() => add(l.id)} className="block w-full text-left px-2 py-1 text-sm hover:bg-gray-50 rounded">
                {l.name}
              </button>
            ))}
            {available.length === 0 && <p className="px-2 py-1 text-xs text-gray-400">No more labels</p>}
            <div className="flex items-center gap-1 mt-1 pt-1 border-t border-gray-100">
              <input
                ...
                className="flex-1 min-w-0 px-2 py-1 text-sm border border-black rounded focus:outline-none focus:ring-2 focus:ring-gray-300"
              />
