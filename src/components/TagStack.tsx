...
import { EVENT_TAGS, tagColor } from "../lib/tags";
...
export function TagStack({
  ...
  const allKnown = Array.from(new Set([...EVENT_TAGS, ...knownTags, ...tags]));
  const q = query.trim().toLowerCase();
  const filtered = allKnown.filter((t) => t.toLowerCase().includes(q));
  const canAdd = q.length > 0 && !allKnown.some((t) => t.toLowerCase() === q);
  ...
  useEffect(() => {
    if (!open) return;
    let cancelled = false;
    getAllTags().then((t) => { if (!cancelled) setKnownTags(t); }).catch(() => {});
          ...
          {tags.map((t, i) => {
            ...
            return (
              <span key={t} className={`inline-flex ${expandOnHover ? "transition-[max-width,margin] duration-200" : ""} ${marginCls} ${widthCls}`}>
                <span className={`inline-block px-3 py-1 rounded-full text-sm whitespace-nowrap ${tagColor(t)} ${highlight}`}>{t}</span>
              </span>
            );
          })}
        </span>
      ...
      {editable && open && pos && createPortal(
        ...
        >
          <div className="bg-white p-1 border-b border-gray-100 flex items-center gap-1">
            <div className="relative flex-1">
              ...
              <input
                ...
                onKeyDown={(e) => { if (e.key === "Enter" && canAdd) select(query.trim()); }}
          ...
          <div className="p-1 overflow-y-auto">
            {canAdd && (
              <button onClick={() => select(query.trim())} className="flex items-center gap-2 w-full text-left px-2 py-1.5 rounded hover:bg-gray-50 text-sm text-gray-800 font-medium">
            ...
            {filtered.length === 0 && !canAdd && <p className="px-2 py-2 text-xs text-gray-400">Type to create a tag</p>}
          </div>
        </div>,
        document.body,
      )}
